setInterval(() => {
  ...
  paragraphsToCheck.forEach(paragraph => {
    if (paragraph.textContent.includes('드') && !paragraph.classList.contains('hm')) {
      elementsToAddMoh.push(paragraph);
    } else if (!paragraph.textContent.includes('드') && paragraph.classList.contains(hm')) {
      paragraph.classList.remove('hm');
    }
  });

  elementsToAddMoh.forEach(element => {
    element.classList.add('hm');
  });

  $("#users .uzr.hm span.uhash, #users .active.hm fl.ustat, #users .uzr.hm img.co, #users .uzr.hm .ustat, .fl.mini.u-co").hide();
